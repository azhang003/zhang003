...
//
//            /** 备份旧表 **/
//            $sql       = "create table ea_game_event_list_" . date('Ymd', time() - 86400) . " select * from ea_game_event_list";
//            $back_bool = Db::execute($sql);
//            if (!$back_bool) {
//                throw new Exception('备份失败!>>' . $back_bool);
//            }
//            /** 将当日订单插入总表 **/
//            $sql         = "insert into ea_game_event_list_old select * from ea_game_event_list_" . date('Ymd', time() - 86400);
//            $insert_bool = Db::execute($sql);
//            if (!$insert_bool) {
//                throw new Exception('插入失败!' . $insert_bool);
//            }
//            /** 清除昨日备份表 **/
//            $sql = "drop table ea_game_event_list_" . date('Ymd', time() - 86400 * 2);
//            Db::execute($sql);
//
//            $is_delete = GameEventList::whereNotLike('title', '%' . date('Ymd', time() - 86400) . '%')->delete();
//            if (!$is_delete) {
...
//            }
        ...
        foreach ($currency as $value) {
            ...
            for ($j = 0001; $j <= 24 * 60; $j++) {
                ...
                $insert['type']         = "1m";
                $insert['cid']          = $value['id'];
                $insert['title']        = $value['id'] . '@' .$insert['type'] . '_' . date('Ymd', time()) . $j;
                ...
                $insert['control_time'] = $insert['end_time'] - 10;
                $insertAll[]            = $insert;
            }
             ...
             * 3min
             */
            for ($j = 0001; $j <= 24 * 20; $j++) {
                $insert                 = [];
                $insert['type']         = "3m";
                $insert['cid']          = $value['id'];
                $insert['title']        = $value['id'] . '@' .$insert['type'] . '_' . date('Ymd', time()) . $j;
                $insert['begin_time']   = strtotime(date('Y-m-d', time())) + ($j - 1) * 3 * 60;
                $insert['end_time']     = strtotime(date('Y-m-d', time())) + $j * 3 * 60;
                $insert['create_time']  = time();
