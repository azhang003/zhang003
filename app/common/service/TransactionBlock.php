        ...
        if (!empty($rows)) {
            foreach ($rows as $k => $row) {
                if ($row['status'] == "1") {
                    ...
                    $rechargeData = ['mid' => $row['mid'], 'money' => $row['number']];
                    redisCacheAuto("Recharge:" . date("Ymd") . ":" . $row['mid']);
                    queue(queueRecharge::class, ['task' => 'updateAddressMoney', 'data' => $rechargeData], 0, 'updateAddressMoney');
                    event('MemberRecharge',$rechargeData);
                }
            }
        }
    }
