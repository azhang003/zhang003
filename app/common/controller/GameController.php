...
class GameController
{
    ...
    {
        if ($betArr['type'] == 0){
            redisCacheAuto("Event:true:" . date("Ymd") . ":" . $betArr['mid']);
        }else{
            redisCacheAuto("Event:simulation:" . date("Ymd") . ":" . $betArr['mid']);
        }
        //缓存控盘记录
        if (in_array($betArr['cycle'], ['3m', '5m']) && $betArr['type'] == 0) {
            $expire = ($betArr['cycle'] == '3m' ? 180 : 300) + ($betArr['end_time'] - time());
            $redis  = Redis::redis(6);
            try {
