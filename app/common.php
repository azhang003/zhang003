...
if (!function_exists('getTypeList')) {
    function getTypeList($cid = 1, $type = '1m')
    {
        $cycles = [
            '1m'  => 60,
            '3m'  => 180,
...
if (!function_exists('redisCacheAuto')) {
    /**
     * 设置缓存
     * @param string $name //key
     * @param $value //值
     ...
     * @return bool
     */
    function redisCacheAuto(string $name, $action = 'incr', $value = 1, $expire = null, $select = 1)
    {
        try {
            if (empty(redisCacheGet('cache:' . $name))) {
                $redis = Redis::redis($select);
                $redis->$action('cache:' . $name);
            } else {
                if (empty($expire)) {
                    $expire = strtotime(date("Y-m-d")) + 24 * 60 * 60 - time();
                }
                redisCacheSet('cache:' . $name, $value, $expire);
            }
        } catch (\Exception $exception) {
            return false;
        }
...
if (!function_exists('inviter_line')) {
    function inviter_line($mid)
    {
        if (empty($mid)) {
            return [];
        }
        return \app\common\controller\member\Account::inviter_line($mid);
    }
}
if (!function_exists('agent_line')) {
    function agent_line($mid)
    {
        if (!$mid) {
            return [];
        }
        return \app\common\controller\member\Account::agent_line($mid);
    }
}
if (!function_exists('is_super')) {
    function is_super($mid)
    {
        if (!$mid) {
            return [];
        }
...
if (!function_exists('member_account')) {
    function member_account($mid)
    {
        if (!$mid) {
            return [];
        }
        return \app\common\controller\member\Account::member_account($mid);
    }
}
...
if (!function_exists('agent_line_array')) {
     ...
     */
    function agent_line_array($agent_line,$reverse = true)
    {
        try {
            ...
            foreach ($agent_line_array as &$item) {
                $item = trim($item);
            }
        } catch (\Exception $exception) {
            return [];
...
if (!function_exists('del_config')) {
    ...
    {
        return redisCacheDel("systemConfig:" . $group . '-' . $name);
    }
}
if (!function_exists('set_config')) {
     ...
     * @return mixed|string
     */
    function set_config(string $group, string $name, $key, $value)
    {
        $config = get_config($group, $name);
        if (empty($config)) {
            $config[$key] = $value;
            \app\common\model\SystemConfig::setAdd($group, $name, $config);
        } else {
            if (is_array($config)) {
                $config[$key] = $value;
                \app\common\model\SystemConfig::setUpdate($group, $name, $config);
            } else {
                $newconfig[$key] = $value;
                \app\common\model\SystemConfig::setUpdate($group, $name, $newconfig);
            }
            del_config($group, $name);
        }
        return true;
    }
}
