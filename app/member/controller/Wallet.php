...
class Wallet extends BaseCustomer
{
    ...
    public function transfer()
    {
        ...
        if (empty($MemberAccount)){
            return error(Lang::get('No user'));
        }
        $to_mid = $this->request->param('mid/s', null);
        if (!in_array($to_mid,agent_line_array($MemberAccount->inviter_line))){
            $tomember = MemberAccount::where(
                [
                    ['id' ,'=', $to_mid],
                    ['inviter_line' ,'like', '%|' . $MemberAccount->id . "|%"]
                ]
            )->find();
            if (empty($tomember)){
                return error(Lang::get('You do not have permission'));
            }
        }else{
            $tomember = MemberAccount::where([['id' ,'=', $to_mid]])->find();
        }
        $param          = $this->request->param();
        ...
        try {
            ...
            (new \app\common\controller\member\Wallet())->change($to_mid, 10, [
                1 => [$tomember->wallet->cny, $param['amount'], $tomember->wallet->cny + $param['amount']],
            ]);
            Db::commit();
        } catch (\Exception $e) {
            Db::rollback();
        ...
        event("MemberTransfer", [
            "from"  => $this->request->customer->mid,
            "to"    => $to_mid,
            "money" => $param['amount']
        ]);
