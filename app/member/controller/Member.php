...
class Member extends BaseCustomer
{
    protected $middleware
        = [
            jwtVerification::class => [
                'except' => ['register', 'login', 'logins', 'index', 'forgotPassword','openshare']
            ]
        ];
    ...
    {
        $data             = [];
        $data['username'] = $this->request->post('username/s', null);
        $data['password'] = $this->request->post('password/s', null);
        $scene            = "MobileLogin";
        ...
        try {
            ...
            if (jwt_realIp()){
                $customer->account()->update(['login_ip' => jwt_realIp()]);
            }
        ...
        event('MemberLogin', [
            "account" => $customer->account->toArray(),
            'profile' => $customerArray,
     ...
     */
    public function memberInfo():Json
    {
        $profile = Account::MemberCache($this->request->customer->mid);
        event('MemberGetInfo',$profile);
...
//        }
    ...
    {
        $inviter = $this->request->param('inviter/s',null);
        if (empty($inviter)){
            return success('');
        }
        $member = MemberAccount::where('uuid',$inviter)->find();
        if (!empty($member)){
            event('MemberOpenShare',$member->toArray());
        }
        return success();
    }
}
