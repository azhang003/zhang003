...
class Game extends BaseMerchant
{
    ...
    {
        $value = $this->request->param('proportion',0.00336);
        set_config('game', 'game', 'proportion',floatval($value));
//        if (in_array($this->request->merchant->id,explode("|",get_config('game', 'game', 'open')?:0))  ){
//        }
        return success();
    }
    ...
    {
        $proportion_list = agent_line_array(get_config('game', 'game', 'proportion_list'),false);
        return success([
            'list' => $proportion_list,
            'proportion' => get_config('game', 'game', 'proportion')?:00336
        ]);
    }
    /**
    ...
    public function update_open()
    {
        try {
            ...
            $list = GameEventList::where([['id', '=', $list_id], ['control_time', '>', time()], ['type', 'in', ['3m', '5m']]])->find();
            if (!$list) {
