...
class Index extends BaseMerchant
{
    ...
    public function user_List(Request $request)
    {
        try {
            ...
            $query   = Db::table('ea_member_account')
                ...
                ->join('ea_member_index i', 'a.id = i.mid')
                ->join('ea_member_wallet w', 'a.id = w.mid')
                ->join('ea_member_team t', 'a.id = t.mid')
                ->join('ea_member_ip_address li', 'a.login_ip = li.ip')
                ->join('ea_member_ip_address ri', 'a.register_ip = ri.ip');
            $where   = [];
            $order   = [];
            $account = [];
            ...
            $agentLine = $this->request->merchant->id;
            if (!empty($uid)) {
                $account = member_account($uid);
                if (empty($account)) {
                    return error('上级不存在!');
                }
                $where[] = ['a.inviter_line', 'like', "%|" . $uid . "|%"];
            } else {
                $where[] = ['a.agent_line', 'like', "%|" . $agentLine . "|%"];
            }
            if (!empty($mobile)) {
            ...
            if (!empty($registration_ip)) {
                $where[] = ['a.login_ip', '=', $registration_ip];
            }
            if (!empty($start_time)) {
                $where[] = ['a.login_time', '>', $start_time];
            }
            if (!empty($end_time)) {
                $where[] = ['a.login_time', '<', $end_time];
            }
            // 1关注用户 2拉黑用户 3重复IP注册 4初代用户 5非初代用户
            if (!empty($user_type)) {
                switch ($user_type) {
            ...
            $order['a.id']  = 'desc';
            $a              = 'a.id,a.type,a.agent_line,a.inviter_line,a.create_time,a.login_ip,a.register_ip,a.status,';
            $p              = 'p.nickname,p.mobile,';
            $d              = 'd.bet,d.bet_amount,d.award_amount,d.recharge_amount,d.withdraw_amount,d.withdraw_free,d.transfer_into_amount,d.transfer_out_amount,';
            $w              = 'w.cny,w.cny_freeze,';
            $t              = 't.all_share,t.all_receive,';
            $li             = 'li.address AS login_ip_address,';
            $ri             = 'ri.address AS register_ip_address';
            $count          = $query->count();
            $data           = $query->where($where)
                ->Field($a . $p . $d . $w . $t . $li . $ri)
                ->page($page)
                ->limit($limit)
                ->order($order)
                ->select();
            ...
            return error($exception->getMessage(), 201, 200, $exception->getTrace());
        }
    }
    ...
    {
        $query = Db::table('ea_merchant_dash')
            ->alias('d')
            ->join('ea_merchant_wallet w', 'w.uid = d.uid');
        $data  = $query->where([
            ['d.uid', '=', $this->request->merchant->id],
        ])->Field('d.*,w.cny')
            ->find();
        return success($data);
    }
...
//            $data = $GameEventBet->where($where)->page($page, $limit)->order($order)->select();
    ...
    public function user_GameData()
    {
        ...
        $query     = Db::table('ea_member_account')
            ->alias('a')
            ->join('ea_member_dash_day d', 'a.id = d.mid')
            ->join('ea_member_profile p', 'a.id = p.mid')
            ->join('ea_member_wallet w', 'a.id = w.mid');
        ...
        $count          = $query->count();
        $data           = $query->where($where)
            ->Field('a.id,d.date,p.nickname,p.mobile,a.agent_line,d.bet,d.bet_amount,d.award_amount,d.recharge_amount,d.withdraw_amount,w.cny')
            ->page($page)
            ->limit($limit)
            ->order($order)
            ->select();
    ...
    public function gameRecord(Request $request)
    {
        ...
        $query         = Db::table('ea_member_account')
            ->alias('a')
            ->join('ea_game_event_bet b', 'a.id = b.mid')
            ->join('ea_member_record r', 'b.id = r.bet')
            ->join('ea_game_event_currency c', 'b.cid = c.id');
        ...
        if (!empty($issue_number)) {
            $where[] = ['b.title', '=', $issue_number];
        }
        if (!empty($game_type)) {
            $type    = ['', '1m', '5m', '15m', '30m', '1h', '1d', '',];
            $where[] = ['b.cycle', '=', $type[$game_type]];
        }
        if (!empty($trading_order)) {
            $where[] = ['b.type', '=', $trading_order];
        }
        if (!empty($game_results)) {
            $where[] = ['b.is_ok', '=', $game_results];
        }
        if (!empty($start_time)) {
        ...
        $count          = $query->count();
        $data           = $query->where($where)
            ->Field('b.*,r.before,r.after,c.title AS cid_title')
            ->page($page)
            ->limit($limit)
            ->order($order)
            ->select();
    ...
    public function now_gamelist()
    {
        ...
        $type = $this->request->param('type/s', '1m');//类型
        if (!in_array($type, ['3m', '5m'])) {
            return error('参数错误!');
        }
        $titles        = getTypeList($cid, $type);
        $GameEventList = new GameEventList();
        $data['game']  = $GameEventList->where('title', $titles['now'])->field('title,id,cid,begin_time,end_time,seal_time,open,type,iscontrol')->find(); //赛事列表数据
        $data['last']  = $GameEventList->where('title', $titles['last'])->field('title,id,cid,begin_time,end_time,seal_time,open,type,iscontrol')->find();//赛事列表数据
        return success($data);
    }
    ...
    public function now_list()
    {
        try {
            ...
            return error($msg = 'error', $errcode = 201, $code = 200, $err = $exception->getTrace());
...
//        $mids = MemberAccount::where([['agent_line', 'like', '%|' . $agentLine . '|%'], ['analog', '=', 0]])->column('id');
    ...
    public function withdrawalLostUserList()
    {
        $page      = $this->request->param('page/d', 1);
        $limit     = $this->request->param('limit/d', 10);
        $mid       = $this->request->param('mid');
        ...
        $query     = Db::table('ea_member_account')
            ...
            ->join('ea_member_wallet w', 'a.id = w.mid')
            ->join('ea_member_team t', 'a.id = t.mid');
        $mix_day   = get_config('wallet', 'wallet', 'mix_day') ?: 3;
        $where     = [['a.login_time', '<', time() - $mix_day * 24 * 60 * 60]];
        ...
        $count          = $query->count();
        $a              = 'a.id,a.type,a.agent_line,a.inviter_line,a.create_time,a.status,a.login_time,';
        $p              = 'p.nickname,p.mobile,';
        $d              = 'd.bet,d.bet_amount,d.award_amount,d.recharge_amount,d.withdraw_amount,d.transfer_into_amount,d.transfer_out_amount,d.withdraw_freeze,';
        $w              = 'w.cny,';
        $t              = 't.all_share,t.all_receive';
        $data           = $query->where($where)
            ->Field($a . $p . $d . $w . $t)
