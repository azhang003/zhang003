...
class Index
{
    public function updateShare(array $dataArr)
    {
        ...
        if (empty($MSD)) {
            $MSDN      = new MemberShareDay();
            $MSDN->mid = $mid;
            $MSDN->date = date('Y-m-d');
            $MSDN->save();
            $MSD = MemberShareDay::where([
                ['mid', '=',$mid],
                ['date', '=',date('Y-m-d')]
        ...
        try {
            switch ($data['type']) {
                case "register":
                    if (array_key_exists('team', $data) && array_key_exists($data['team'], self::$teamType)) {
                        $team = self::$teamType[$data['team']];
                        $MS->people  = Db::raw('people+1');
                        $MS->$team  = Db::raw($team.'+1');
                        $MSD->people = Db::raw('people+1');
                        $MSD->$team  = Db::raw($team.'+1');
                    }
                    break;
                case "bet":
                    var_dump($data);
                    ...
                    if (array_key_exists('team', $data) && array_key_exists($data['team'], $teamType)) {
                        $teamQuantity      = $teamType[$data['team']] . '_' . 'quantity';
        ...
        var_dump($MS->bet);
        var_dump($MS->bet_amount);
        var_dump($MSD->bet);
        var_dump($MSD->bet_amount);
        return success([]);
    }

    public function pushBonus(array $data)
...
//                }
    ...
    public function commission()
    {
        ...
        $sql            = "SELECT  first_share - first_receive AS '一级',second_share - second_receive AS '二级',third_share - third_receive AS '三级',vip -vip_receive AS '四级' FROM `coco`.`ea_member_team` WHERE `mid` = '" . $mid . "'";
        $haveCommission = Db::query($sql);
        if (count($haveCommission) > 0) {
            $haveCommission = $haveCommission[0];
        }
        $data = [
...
//            $token = 'kaadon eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJpc3MiOiJrYWFkb24iLCJpYXQiOjE2NjEyMjQyNzMsImV4cCI6MTY2MTgyOTA3MywiZGF0YSI6eyJ0eXBlIjoiY3VzdG9tZXIiLCJpZCI6NDUwMDQ5LCJhZ2VudF9saW5lIjoiMHwyfDR8IiwidXVpZCI6IkgzODEyOSIsIm1pZCI6NDUwMDQ5LCJpZGVudGlmaWNhdGlvbiI6IkgzODEyOSIsImlwIjoiNjEuMTExLjEyOS4xNzMifX0.mgaU4NNNvqhPNUWmRMNsqaOULj3gluouL3jEaB85fd3fr7LOrbALTm22RbuL9Iol9FwVuSchJQXTMRjOZn4bX4WHgJSNhp29u19jbrMvhYvt99fVYcJJx1S6XAps4eWMmXs_S6jkDElGx5srdrc0f0iz326OBWeXCcCuxIRJ89A';
    ...
    public function curr_list()
    {
        ...
        foreach ($stock as $item) {
            $currencyData = $redis->get($item);
            if (!empty($currencyData)) {
                $currencyData                    = json_decode($currencyData, true);
                $data[$currencyData['currency']] = $currencyData;
            }
        }
        return success($data);
    }
